...
export function TangkapIkan({ onSelesai }: { onSelesai: (skor: number) => HasilAksi }) {
  ...
  useEffect(() => {
    if (fase !== "main") return;
    const tick = setInterval(() => setSisa((s) => (s <= 1 ? 0 : s - 1)), 1000);
    const spawn = () => {
      setIkan((arr) => {
      ...
      <GameIntro
        ...
        info={
          <div className="flex flex-wrap items-center justify-center gap-2 font-body text-xs font-semibold text-navy">
            <span className="inline-flex items-center gap-1 rounded-full bg-white/80 px-2.5 py-1 shadow-sm">🐟 +1</span>
            <span className="inline-flex items-center gap-1 rounded-full bg-white/80 px-2.5 py-1 shadow-sm">🐠 +3</span>
            <span className="inline-flex items-center gap-1 rounded-full bg-white/80 px-2.5 py-1 shadow-sm">🥾 −2</span>
          </div>
        }
      />
    );
  ...
  if (fase === "habis")
    return (
      <GameResult skorLabel="Kamu menangkap" skor={skor} satuan="poin" pesan={hasilPesan} onUlang={mulai} />
    );
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-3">
        ...
        <span className="w-9 text-right font-display text-lg font-extrabold text-navy">{sisa}s</span>
      </div>
